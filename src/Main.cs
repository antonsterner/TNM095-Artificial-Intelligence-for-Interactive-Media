    ...
    public class Main : MonoBehaviour
    {
        ...
        IEnumerator carUpdate()
        {
            while (true)
            {
                ...
                {  
                    cars.CarList[key].UpdateRays(); // fetch ray distance input
                    
                    float rayValue; // save one input ray value 
                    ...
                    while (outputCounter < nrOutputs)
                    {
                        ...
                        {
                            NetworkEvaluator.genomes[key].Nodes[NetworkEvaluator.genomes[key].Connections[con].OutNode].sum = 
                                (float)Math.Tanh(NetworkEvaluator.genomes[key].Nodes[NetworkEvaluator.genomes[key].Connections[con].OutNode].sum);
                        }                            
                    ...
                    if (cars.CarList[key].hasTouched || (timeSinceInit > 15 && carDistance[key] < 15) || timeSinceInit > 60)
                    {
                        ...
                        NetworkEvaluator.genomes[key].fitness = carDistance[key] * carDistance[key] / timeSinceInit;
